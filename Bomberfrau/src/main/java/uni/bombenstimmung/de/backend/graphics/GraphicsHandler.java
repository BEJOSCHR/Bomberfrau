...
public class GraphicsHandler {
    ...
    /**
     * Wird am anfang aufgerufen um sowohl den Frame als auch das Label zu erzeugen
     * und zuzuordnen
     */
	...
	frame.setLocation((Settings.getResWidthMax() - Settings.getResWidth()) / 2,
...
//		frame.setSize(1920, 1080);
    ...
    /**
     * Deaktiviert die M�glichkeit der Fensterbewegung bei Vollbild, aktivert sie
     * ansonsten.
     */
	...
	// SoundHandler.playSound(SoundType.INTRO, false,
	// Menu.VolumeIntToFloat(Settings.getIni_VolMusic()));
...
//	        Menu.titlePulseAni();
	...
	// SoundHandler.playSound(SoundType.MENU, false,
	// Menu.VolumeIntToFloat(Settings.getIni_VolMusic()));
	...
	if (DeadPlayerHandler.getClientPlayer().isHost()) {
	    ...
	    lobby = new LobbyCreate(new LobbyPlayer(DeadPlayerHandler.getClientPlayer().getName(),
		    DeadPlayerHandler.getClientPlayer().getIp()), true);
	}
    ...
    /**
     * Wird aufgerufen wenn w�hrend einem Spiel das Spiel verlassen wird bzw der
     * Host das Spiel schlie�t
     */
    ...
    /**
     * Wird aufgerufen wenn aus dem Menu ein Spiel erstellt wird oder einem
     * beigetreten wird
     */
...
//		    lobby = new LobbyCreate(new LobbyPlayer(Settings.getUserName(), Settings.getIp()));
	...
	if (!switchToIngame) {
		...
		if (i == 0) {
		    PlayerHandler.addPlayerFromLobby(LobbyCreate.player[i].getId(), LobbyCreate.player[i].getName(),
			    LobbyCreate.player[i].getIpAdress(), LobbyCreate.player[i].getisHost(),
			    LobbyCreate.player[i].getSkin(), new Point(1, 1), LobbyCreate.client);
		}
		if (i == 1) {
		    PlayerHandler.addPlayerFromLobby(LobbyCreate.player[i].getId(), LobbyCreate.player[i].getName(),
			    LobbyCreate.player[i].getIpAdress(), LobbyCreate.player[i].getisHost(),
			    LobbyCreate.player[i].getSkin(), new Point(15, 1), LobbyCreate.client);
		}
		if (i == 2) {
		    PlayerHandler.addPlayerFromLobby(LobbyCreate.player[i].getId(), LobbyCreate.player[i].getName(),
			    LobbyCreate.player[i].getIpAdress(), LobbyCreate.player[i].getisHost(),
			    LobbyCreate.player[i].getSkin(), new Point(1, 15), LobbyCreate.client);
		}
		if (i == 3) {
		    PlayerHandler.addPlayerFromLobby(LobbyCreate.player[i].getId(), LobbyCreate.player[i].getName(),
			    LobbyCreate.player[i].getIpAdress(), LobbyCreate.player[i].getisHost(),
			    LobbyCreate.player[i].getSkin(), new Point(15, 15), LobbyCreate.client);
		}
	    }
	    ...
	    // Alle Player Objekte der Lobby loeschen, sodass man es resettet ist fuers
	    // Aftergame
    ...
    /**
     * Wird aufgerufen wenn ein Spiel beendet wird und man das Aftergame betreten
     * soll
     */
	...
	DeadPlayerHandler.setClientPlayer(PlayerHandler.getClientPlayer().getId(),
		PlayerHandler.getClientPlayer().getName(), PlayerHandler.getClientPlayer().getIpAdress(),
		PlayerHandler.getClientPlayer().getHost(), PlayerHandler.getClientPlayer().getSkin(),
	...
	if (DeadPlayerHandler.getClientPlayer().isHost()) {
		...
		DeadPlayerHandler.addDeadPlayer(PlayerHandler.getAllPlayer().get(i).getId(),
    ...
    /**
     * Der einzige saubere Weg dieses Programm zu stoppen (Stoppt alle Timer und
     * schlie�t KONTROLLIERT alle Datenzug�nge bzw speichert setting etc). Wenn
     * einmal aufgerufen werden weitere Aufrufe dieser Methode abgeblockt, so dass
     * ein doppeltes runterfahren nicht m�glich ist!
     */
