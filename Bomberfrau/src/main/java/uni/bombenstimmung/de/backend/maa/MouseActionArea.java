...
public class MouseActionArea {
     ...
     * @param type          - Der Type der diese Area von den anderen Unterscheidet
     *                      bzw zum identifizieren genutzt wird
     ...
     **/
    public MouseActionArea(int x, int y, int width, int height, MouseActionAreaType type, String displayText,
	    int textSize, Color standardColor, Color hoverColor) {
     ...
     * @return boolean - True wenn sich die Koordinaten in der Area befindet, false
     *         wenn nicht (Auch false wenn nicht aktive!)
     **/
     ...
     * 
     * @return boolean - Den selben status von checkArea() nur mit den
     *         Mouse-Koorinaten als Input
     **/
    ...
    /**
     * Überprüft ob diese Area gerade aktiv ist. (Soll überschrieben/verändert
     * werden wenn eine neue MAA erzeugt wird (@Overwrite))
     * 
     * @return boolean - True wenn die Aktivierungsbedingung erfüllt ist, sont false
     *         (Wenn nicht überschrieben/verändert dann standardmäßig true)
     **/
    ...
    /**
     * Wird aufgerufen wenn diese MAA mit der LINKEN Taste GEDRÜCKT wird - LEFT
     * PRESS (Soll überschrieben/verändert werden wenn eine neue MAA erzeugt wird
     * (@Overwrite))
     **/
    ...
    /**
     * Wird aufgerufen wenn diese MAA mit der RECHTEN Taste GEDRÜCKT wird - RIGHT
     * PRESS (Soll überschrieben/verändert werden wenn eine neue MAA erzeugt wird
     * (@Overwrite))
     **/
    ...
    /**
     * Wird aufgerufen wenn diese MAA mit der LINKEN Taste LOSGELASSEN wird - LEFT
     * RELEASE (Soll überschrieben/verändert werden wenn eine neue MAA erzeugt wird
     * (@Overwrite))
     **/
    ...
    /**
     * Wird aufgerufen wenn diese MAA mit der RECHTEN Taste LOSGELASSEN wird - RIGHT
     * RELEASE (Soll überschrieben/verändert werden wenn eine neue MAA erzeugt wird
     * (@Overwrite))
     **/
     ...
     * aktiv ist) Standardmäßig wird der Rand der MAA in der jeweiligen Farbe
     * (Abhängig ob gehovered oder nicht) mit dem dispplaytext dargestellt Custom
     * veränderungen können in der drawCustomParts methode überschrieben/verändert
     * (@Overwrite) werden
     * 
    ...
    /**
     * Stellt zusätzliche grafische Veränderungen dar (Soll überschrieben/verändert
     * werden wenn eine neue MAA erzeugt wird (@Overwrite))
     * 
     * @param g - {@link Graphics} - Der Grafik-Parameter
     */
