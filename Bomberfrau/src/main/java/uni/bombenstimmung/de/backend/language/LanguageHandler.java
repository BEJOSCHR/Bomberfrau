...
public class LanguageHandler {
	...
	String[] menu_info1 = { "min. " + Menu.getMinNameLength() + " signs",
		"min. " + Menu.getMinNameLength() + " Zeichen" };
	new LoadedLanguageBlock(LanguageBlockType.LB_MENU_INFO1, menu_info1);
	String[] menu_info2 = { "max. " + Menu.getMaxNameLength() + " signs",
		"max. " + Menu.getMaxNameLength() + " Zeichen" };
	...
	String[] options_info2 = { "All controls have to be set before leaving.",
		"Die Steuerung muss vollstaendig belegt sein." };
	...
	String[] bad_name = { "Please enter a valid name before starting ...",
		"Bitte vor dem Start einen gueltigen Namen eingeben ..." };
	...
	String[] bad_res = { "resolution to high - switching to fullscreen.",
		"Die Aufloesung ist zu gross - Wechsel zu Vollbild." };
	...
	String[] lobby_full = { "This lobby is full or does not exist! Returning to main menu...",
		"Diese Lobby ist entweder voll oder sie existiert nicht! Gehe zurueck ins Hauptmenu..." };
    ...
    /**
     * Fügt den LLB zur languageBlocks Liste hinzu (Wird eigentlich nur aus einem
     * LLB aufgerufen)
     * 
    ...
    /**
     * Gibt den LLB zum angegebenen Type zurück (Wenn richtig geladen gibt es für
     * jeden Type einen LLB)
     * 
     * @param type - Der {@link LanguageBlockType} zu dem der gesuchte LLB gehört
     * @return Den {@link LoadedLanguageBlock} der durch den Type identifiziert
     *         wird, falls keiner gefunden wird null
     */
