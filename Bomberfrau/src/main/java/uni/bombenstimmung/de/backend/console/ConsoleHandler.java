...
public class ConsoleHandler {
    ...
    /**
     * Gibt die angegebene message via print(message, type) aus (MessageType =
     * NONESPECIFIC)
     * 
    ...
    /**
     * Gibt die angegebene message mit dem angegebenen type aus, wenn der Type dem
     * derzeitigen debugType entspricht oder sich über diesen hinweg setzen kann. So
     * wird ERROR immer ausgegeben und IMPORTANT wird nur nicht ausgegeben wenn
     * currentDebugType = ERROR ist
     * 
     * @param message - Die Message die ausgegeben werden soll
     * @param type    - Der {@link MessageType} der Nachricht, der die zugehörigkeit
     *                der message defeniert
     */
    ...
    /**
     * Startet den Scanner der auf Eingaben in der Console wartet und diese
     * weiterleitet. Wenn bereits ein Scanner gestartete wurde wird kein weiterer
     * gestartet
     */
	...
	if (inputScanner == null) {
	    ...
	    ConsoleHandler.print("Couldn't init/start Inputscanner, because there is already one running!",
    ...
    /**
     * Stoppt den Scanner der auf Eingaben in der Console wartet und diese
     * weiterleitet, falls dieser läuft.
     */
	...
	case "host":
	    ...
	    ConsoleHandler.print(
		    "Server is Host: " + host.isHost() + " with " + host.hostGetPublicIP() + ":" + ConnectionData.PORT,
		    MessageType.BACKEND);
	    break;
	case "client":
	    if (input.length >= 2) {
		ConsoleHandler.print("Connecting to server...", MessageType.IMPORTANT);
		ConnectedClient client = new ConnectedClient(false, input[1]);
		ConsoleHandler.print("Client is Host: " + client.isHost(), MessageType.BACKEND);
	    } else {
	...
	case "language":
		...
		try {
		    ...
		    ConsoleHandler.print("Switched Activelanguage to: " + LanguageHandler.getActiveLanguage(),
			    MessageType.IMPORTANT);
		} catch (IllegalArgumentException error) {
		...
		ConsoleHandler.print("Use: language [LanguageType] - Current: " + LanguageHandler.getActiveLanguage(),
			MessageType.IMPORTANT);
	    }
	    break;
	case "help":
    ...
    /**
     * Ändert den derzeitigen DebugType, abhängig davon werden manche ausgeben vor
     * der ausgabe blockiert
     * 
