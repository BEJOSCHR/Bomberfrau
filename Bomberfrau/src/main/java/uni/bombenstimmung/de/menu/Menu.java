	 ...
	 */
        public static void buildMenu() {
        	...
        	name_info.setHorizontalAlignment(JLabel.CENTER);
                name_info.setBounds((int)(Settings.res_width*0.52), (int)(Settings.res_height*0.37), (int)(Settings.res_width/5), (int)(Settings.res_height/12));
		...
		if (Settings.create_selected) ip_info.setVisible(false);
            	if (!checkIp(false))
            	    ip_info.setText(LanguageHandler.getLLB(LanguageBlockType.LB_MENU_INFO2).getContent());
            	ip_info.setBackground(Color.WHITE);
        	ip_info.setForeground(Color.RED);
        	ip_info.setFont(GraphicsHandler.usedFont(20));            	
            	ip_info.setBounds((int)(Settings.res_width*0.52), (int)(Settings.res_height*0.6), (int)(Settings.res_width/5), (int)(Settings.res_height/12));
	    ...
	    Settings.factor = (float)(Settings.res_height)/Settings.res_height_max;
            ...
            sliderSound.addChangeListener(new ChangeListener() {
            	public void stateChanged(ChangeEvent event) {
            	    ...
            	    FloatControl volume = (FloatControl) SoundHandler.getSound(SoundType.OPTIONS).getClip().getControl(FloatControl.Type.MASTER_GAIN);
...
/****************************  INIT MAA MENÜ  **********************************************************************************************************************/
	...
	/**
	 * Knöpfe/MAAs des Hauptmenüs:
	 * - Start:   Übergang in die Lobby nach Überprüfung von Namen und IP Adresse
	 * - Options: Übergang zu den Optionen
	 * - Exit:    Beenden des Spieles
	 */
		...
		start = new MouseActionArea((int)(Settings.res_width*0.1), (int)(Settings.res_height*0.49), (int)(Settings.res_width*0.2), (int)(Settings.res_height*0.085),
			...
			public void performAction_LEFT_RELEASE() {
			          ...
			          Panes.InfoPane(null, "Name \"" + Settings.user_name + "\" + IP \"" + Settings.ip + "\" saved in save.ini. Now switching to Lobby","OK");
			        ...
			        //SoundHandler.reduceAllSounds();
		                ...
		                GraphicsHandler.switchToLobbyFromMenu();
		                Panes.InfoPane(null,"Name \"" + Settings.user_name + "\" saved in save.ini. Now switching to Lobby","OK");
...
/****************************  HILFSMETHODEN  **********************************************************************************************************************/
	...
	/**
	 * Überprüfung des eingegebenen Namens
	 * false, wenn leer oder "?"
	 */
	...
	/**
	 * Überprüfung des Musters der eingegebenen IP-Adresse
	 * false, wenn nicht folgendes Muster vorliegt:  x.x.x.x mit x von 0 bis 255
	 */
	...
	/**
	 * Überprüfung der eingegebenen IP-Adresse
	 * false, wenn leer oder ungültiges Muster
	 */
	...
	/**
	 * Ersetzt Zeichen der Steuerungen
	 * wandelt Kleinbuchstaben in große um 
	 * was weder Buchstabe noch Zahl ist wird durch ein passendes Wort ersetzt
	 */
        private static String codeToText(int i) {
          ...
          if (i == 37) return LanguageHandler.getLLB(LanguageBlockType.LB_KEY_LEFT).getContent();
          if (i == 38) return LanguageHandler.getLLB(LanguageBlockType.LB_KEY_UP).getContent();
          if (i == 39) return LanguageHandler.getLLB(LanguageBlockType.LB_KEY_RIGHT).getContent();
          if (i == 40) return LanguageHandler.getLLB(LanguageBlockType.LB_KEY_DOWN).getContent();
